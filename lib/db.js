...
function initDb() {
  ...
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_wallets_user ON wallets(tg_user_id);
      CREATE INDEX IF NOT EXISTS idx_wallets_user_chain_active ON wallets(tg_user_id, chain, is_active);
...
function upsertWallet({ tgUserId, chain, address }) {
  const now = Date.now();
  // Deactivate previous active wallet for the same chain only
  db.prepare(`UPDATE wallets SET is_active=0 WHERE tg_user_id=? AND chain=? AND is_active=1`)
    .run(String(tgUserId), chain);

  // Insert new active wallet (keeps history)
  db.prepare(`
...
function saveActiveWallet({ tgUserId, chain, address }) {
  return upsertWallet({ tgUserId, chain, address });
}

function listActiveWallets({ tgUserId }) {
  const stmt = db.prepare(`
    SELECT w.*
    FROM wallets w
    JOIN (
      SELECT chain, MAX(id) AS max_id
      FROM wallets
      WHERE tg_user_id=? AND is_active=1
      GROUP BY chain
    ) latest
    ON latest.chain = w.chain AND latest.max_id = w.id
    WHERE w.tg_user_id=? AND w.is_active=1
    ORDER BY w.chain ASC
  `);
  return stmt.all(String(tgUserId), String(tgUserId));
}

function getActiveWallet({ tgUserId, chain }) {
  if (chain) {
    const stmt = db.prepare(`
      SELECT * FROM wallets
      WHERE tg_user_id=? AND chain=? AND is_active=1
      ORDER BY id DESC LIMIT 1
    `);
    return stmt.get(String(tgUserId), chain) || null;
  }
  // Legacy: return most recently added active wallet across chains
  const stmt = db.prepare(`
    SELECT * FROM wallets
    WHERE tg_user_id=? AND is_active=1
    ORDER BY id DESC LIMIT 1
...
function deactivateWallet({ tgUserId, chain }) {
  const stmt = db.prepare(`
    UPDATE wallets SET is_active=0
    WHERE tg_user_id=? AND chain=? AND is_active=1
  `);
  stmt.run(String(tgUserId), chain);
}
